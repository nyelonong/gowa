...
pub const WorkerResult = union(enum) {
    ok: struct { status: u16, body_len: u32 },
...
fn fetchWorker(io: Io, app: *AppState, queue: *std.Io.Queue(WorkerResult)) void {
    ...
    const result: WorkerResult = blk: {
        const fetched = http_engine.fetch(io, arena_state.allocator(), app.method(), url, app.pending_body_buf.len) catch |e| {
        ...
        break :blk .{ .ok = .{ .status = fetched.status, .body_len = @intCast(n) } };
    };
